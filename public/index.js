/* global addEventListener, URLS, Respons, AUTH_SECRET  */
...
const handleGet = async ({ hash }) => {
  ...
  return location ? respondWith({ status: 302, location }) : respondWith({ status: 404 });
};
...
const checkAuth = ({ auth }, callback) =>
  auth === AUTH_SECRET ? callback() : respondWith({ status: 401 });
